...
namespace popo.Database
{
    public class SQLiteHelper6
    {
        ...
        public static class GrandTotalManager
        {
            private static int _grandTotal;

            public static int GrandTotal => _grandTotal;

            public static void SetGrandTotal(int total)
            {
                _grandTotal = total;
            }
        }

        // Function to calculate grandTotal
        public async Task<int> CalculateGrandTotal(int transactionId)
        {
            // Perform the join operation without inserting into the database.
            var orders = await App.OrderedItemsDatabase.ReadOrders();
            var transactions = await App.TransactionDatabase.ReadTransactions();
